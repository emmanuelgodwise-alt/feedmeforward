import { db } from '@/lib/db';
import { NextRequest, NextResponse } from 'next/server';

// GET /api/messages/[userId] — Get conversation with specific user
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ userId: string }> }
) {
  try {
    const userId = request.headers.get('x-user-id');
    if (!userId) {
      return NextResponse.json({ success: false, error: 'Authentication required' }, { status: 401 });
    }

    const { userId: targetUserId } = await params;

    // Validate limit
    const { searchParams } = new URL(request.url);
    let limit = parseInt(searchParams.get('limit') || '50', 10);
    limit = Math.max(1, Math.min(200, limit));

    const before = searchParams.get('before');

    // Build where clause
    const where: Record<string, unknown> = {
      OR: [
        { senderId: userId, receiverId: targetUserId },
        { senderId: targetUserId, receiverId: userId },
      ],
    };

    // Cursor-based pagination
    if (before) {
      const beforeMessage = await db.message.findUnique({ where: { id: before } });
      if (beforeMessage) {
        (where.OR as unknown as Array<Record<string, unknown>>).forEach((condition) => {
          condition.createdAt = { lt: beforeMessage.createdAt };
        });
      }
    }

    // Mark messages as read where receiverId=currentUser and senderId=targetUser
    await db.message.updateMany({
      where: {
        receiverId: userId,
        senderId: targetUserId,
        isRead: false,
      },
      data: { isRead: true },
    });

    // Fetch messages
    const messages = await db.message.findMany({
      where,
      take: limit + 1, // Fetch one extra to determine hasMore
      orderBy: { createdAt: 'desc' },
      include: {
        sender: {
          select: { id: true, username: true, displayName: true, avatarUrl: true },
        },
      },
    });

    const hasMore = messages.length > limit;
    const trimmedMessages = hasMore ? messages.slice(0, limit) : messages;

    return NextResponse.json({
      messages: trimmedMessages,
      hasMore,
    });
  } catch (error) {
    console.error('GET /api/messages/[userId] error:', error);
    return NextResponse.json({ success: false, error: 'Failed to fetch conversation' }, { status: 500 });
  }
}

// DELETE /api/messages/[userId] — Delete a message by ID
// The dynamic param serves as the message ID for DELETE
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ userId: string }> }
) {
  try {
    const userId = request.headers.get('x-user-id');
    if (!userId) {
      return NextResponse.json({ success: false, error: 'Authentication required' }, { status: 401 });
    }

    const { userId: messageId } = await params;

    // Find the message
    const message = await db.message.findUnique({
      where: { id: messageId },
    });

    if (!message) {
      return NextResponse.json({ success: false, error: 'Message not found' }, { status: 404 });
    }

    // Verify message belongs to current user (senderId must match)
    if (message.senderId !== userId) {
      return NextResponse.json({ success: false, error: 'You can only delete your own messages' }, { status: 403 });
    }

    await db.message.delete({
      where: { id: messageId },
    });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('DELETE /api/messages/[userId] error:', error);
    return NextResponse.json({ success: false, error: 'Failed to delete message' }, { status: 500 });
  }
}
