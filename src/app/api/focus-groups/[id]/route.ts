...
// GET /api/focus-groups/[id] — Get single listing with full details
export async function GET(
  ...
  try {
    ...
    const listing = await db.focusGroupListing.findUnique({
      where: { id },
      include: {
        creator: {
          select: {
    ...
    if (userId) {
      const app = await db.focusGroupApplication.findUnique({
        where: {
    ...
    console.error('GET /api/focus-groups/[id] error:', error);
